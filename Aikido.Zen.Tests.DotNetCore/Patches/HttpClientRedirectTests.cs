...
using Moq;
using DotNetZen = Aikido.Zen.DotNetCore.Zen;
...
namespace Aikido.Zen.Tests.DotNetCore.Patches
{
    ...
    {
        private const string HarmonyId = "com.aikido.zen.tests.dotnetcore.httpclient.redirects";
        ...
        private static readonly object[] SsrfUrls =
        {
            ...
            new object[] { "http://ssrf-rédirects.testssandbox.com/ssrf-test" },
            new object[] { "http://xn--ssrf-rdirects-ghb.testssandbox.com/ssrf-test" },
            new object[] { "http://ssrf-rédirects.testssandbox.com/ssrf-test-domain-twice" },
        ...
        public void SetUp()
        {
            ...
            _originalBlock = Environment.GetEnvironmentVariable("AIKIDO_BLOCK");
            ...
            _serviceProvider = CreateServiceProvider(_httpContextAccessor);
            DotNetZen.Initialize(_serviceProvider, _httpContextAccessor);
        ...
        public void TearDown()
        {
            ...
            DotNetZen.Initialize(null, null);
            _httpContextAccessor.HttpContext = null;
            _serviceProvider?.Dispose();
            _agent?.Dispose();

            Environment.SetEnvironmentVariable("AIKIDO_TOKEN", _originalToken);
            Environment.SetEnvironmentVariable("AIKIDO_BLOCK", _originalBlock);
        ...
        private static ServiceProvider CreateServiceProvider(IHttpContextAccessor httpContextAccessor)
        {
            ...
            var contextAccessorType = typeof(DotNetZen).Assembly.GetType("Aikido.Zen.DotNetCore.ContextAccessor");
            var contextAccessor = Activator.CreateInstance(
                contextAccessorType,
                BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public,
                binder: null,
                args: new object[] { httpContextAccessor },
                culture: null);

            services.AddSingleton(contextAccessorType, contextAccessor);
        ...
        {
            _httpContextAccessor.HttpContext.Items["Aikido.Zen.Context"] = new Context
            {
                Method = "GET",
                Route = "/outbound",
        ...
        private async Task AssertSsrfBlockedAsync(string userInputUrl, string requestUrl, string serverUrl)
        {
            ...
            var exception = Assert.ThrowsAsync<AikidoException>(async () => await client.GetAsync(requestUri));
