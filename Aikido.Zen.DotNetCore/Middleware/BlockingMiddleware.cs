...
namespace Aikido.Zen.DotNetCore.Middleware
{
    ...
    internal class BlockingMiddleware : IMiddleware
    {
        public async Task InvokeAsync(HttpContext context, RequestDelegate next)
        {
            ...
            if (Agent.Instance.Context.IsBlocked(user, context.Connection?.RemoteIpAddress?.ToString(), routeKey, aikidoContext.Url, aikidoContext.UserAgent, out var reason))
            {
