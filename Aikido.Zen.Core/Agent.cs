...
namespace Aikido.Zen.Core
{
    ...
    public class Agent : IAgent, IDisposable
    {
        ...
        /// <param name="source">The source of the attack. Can be null for stored SSRF when there is no request-scoped source.</param>
        /// <param name="payload">The attack payload. Can be null for stored SSRF when no user-controlled payload is available.</param>
        /// <param name="operation">The operation where the attack was detected</param>
        /// <param name="context">The context of the attack. Can be null for stored SSRF when the attack is detected outside a live inbound request.</param>
        ...
        {
            LogHelper.AttackLog(Logger, $"Attack detected: {kind} in {(source?.ToString() ?? "unknown source")} {operation}, blocked: {blocked}");
