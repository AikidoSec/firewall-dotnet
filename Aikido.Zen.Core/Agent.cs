...
namespace Aikido.Zen.Core
{
    ...
    public class Agent : IDisposable
    {
        ...
        public void Start()
        {
            ...
            (evt, response) =>
            {
                if (response.Success)
                {
                    ...
                    Task.Run(() => UpdateConfig(reportingResponse.Block, reportingResponse.BlockedUserIds, reportingResponse.AllowedIPAddresses, reportingResponse.Endpoints, reportingResponse.BlockedUserAgentsRegex, reportingResponse.ConfigUpdatedAt));
                }
            });
            ...
            while (!_cancellationSource.Token.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (_lastConfigCheck + TimeSpan.FromMinutes(1).Ticks < DateTime.UtcNow.Ticks)
                    {
                        ...
                        {
                            await UpdateConfig(response.Block, response.BlockedUserIds, response.AllowedIPAddresses, response.Endpoints, response.BlockedUserAgentsRegex, response.ConfigUpdatedAt);
                        }
        ...
        private async Task UpdateConfig(bool block, IEnumerable<string> blockedUsers, IEnumerable<string> allowedIpAdresses, IEnumerable<EndpointConfig> endpoints, Regex blockedUserAgents, long configVersion)
        {
            _context.UpdateConfig(block, blockedUsers, allowedIpAdresses, endpoints, blockedUserAgents, configVersion);
